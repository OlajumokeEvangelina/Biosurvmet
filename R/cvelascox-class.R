...
#'
#' @name cvelascox-class
#' @rdname cvelascox-class
...
#' @slot lamda A vector of estimated optimum lambda for each iterations.
...
#' @author Ziv Shkedy
#' @seealso \code{\link[Biosurvmet]{EstimateHR}}, \code{\link[glmnet]{glmnet}}, \code{\link[Biosurvmet]{Lasoelascox}}
#' @examples
#' ## GENERATE SOME METABOLIC SURVIVAL DATA WITH PROGNOSTIC FACTORS
#' Data<-MSData(nPatients=100,nMet=150,Prop=0.5)
#'
#' ## USE THE FUNCTION
#' Eg = Results = CVLasoelacox(Survival = Data$Survival,Censor = Data$Censor,
#' Mdata = t(Data$Mdata), Prognostic = Data$Prognostic, Quantile = 0.5,
#' Metlist = NULL,Standardize = TRUE, Reduce=FALSE,Select=15,
#' Alpha = 1,Fold = 4,N.cv = 10,nlambda = 100)
#'
#' ## GET THE CLASS OF THE OBJECT
#' class(Eg)     # An "cvelascox" Class
#'
...
setClass("cvelascox",representation(Coef.mat="matrix",Runtime="vector",lambda="vector",n="vector",Met.mat="matrix",HRTrain="matrix",HRTest="matrix",pld="vector",Mdata="matrix"),
         prototype=list(Coef.mat=matrix(1,1,1),Runtime=c(NA),lambda=c(NA), n=c(NA), Met.mat=matrix(1,1,1),HRTrain=matrix(1,1,1) , HRTest=matrix(1,1,1) , pld=c(NA),Mdata=matrix(1,1,1))
)
...
#' @rdname cvelascox-class
#' @aliases show,cvelascox-method
setMethod("show",signature="cvelascox"
          , function(object){
...
#' @rdname cvelascox-class
#' @aliases summary,cvelascox-method
setMethod("summary",signature="cvelascox"
          , function(object){
...
#' @rdname cvelascox-class
#' @aliases plot,cvelascox-method
          setMethod(f ="plot", signature(x="cvelascox", y="missing"),
                    function(x,  y, type=1, ...) {
                      if (class(x)!="cvelascox") stop("Invalid class object")
                      if (type==1) {
