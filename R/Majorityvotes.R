...
#' @param Result An object obtained from the metabolite specific analysis (\code{\link[Biosurvmet]{MSpecificCoxPh}}) which is of class "MSpecific"
...
#' @param K The kth set of patients required for the visualization. The default is K=1 which is the first set of patients. For visualization, K should be less than the number of patients divided by 25
...
#'
#' ## USING THE FUNCTION
#'new = MajorityVotes(Example1,Prognostic, Survival,Censor,J=2)
#'
...
MajorityVotes<-function(Result,Prognostic, Survival,Censor,J=1){
  ...
  if (is.null(Prognostic)) {
    ...
    m0 <- coxph(Surv(Survival, Censor==1) ~ ggr,data=cdata)
  }
  ...
  if (!is.null(Prognostic)) {
    if (is.data.frame(Prognostic)) {
      ...
      eval(parse(text=paste( "m0 <-coxph(Surv(Survival, Censor==1) ~ ggr",paste("+",NameProg[1:nProg],sep="",collapse =""),",data=cdata)" ,sep="")))
    } else {
